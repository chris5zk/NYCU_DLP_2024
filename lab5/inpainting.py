...
class MaskGIT:
    ...
    @staticmethod
    def prepare(root):
        os.makedirs(root + '/' + "test_results", exist_ok=True)
        os.makedirs(root + '/' + "mask_scheduling", exist_ok=True)
        os.makedirs(root + '/' + "imga", exist_ok=True)
...
# mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self, image, mask_b, i):                 # MakGIT inference
        # input: image -> (1, 3, 64, 64), mask_b -> (1, 16*16)  
...
class MaskedImage:
    ...
    def get_mask_latent(self,mask):    
        ...
        return mask_b   # boolean mask from GT mask latent
